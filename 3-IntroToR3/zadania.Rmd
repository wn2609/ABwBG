...
Pole trójkąta obliczamy za pomocą wzoru $a*h/2$. Przypiszmy zmienne:
...
Resztę z dzielenia obliczamy za pomocą funkcji:
```{r}
reszta <- b%%c
```
...
print(reszta)
```
Dzielenie całkowite wykonujemy za pomocą funkcji:
```{r}
...
```{r}
elementy <- c(1:100)
losowe <- sample(elementy,size=50, replace=FALSE)
```
...
Definiujemy wektor:
```{r}
czestosc <- c(3, 7, 7, 2, 2, 5)
```

Tworzymy tabelę częstości:
```{r}
tabela <- table(czestosc)
```
...
```{r}
dane_ramka$wartość <- dane_ramka$cena * dane_ramka$ilość
```
...
Przypisujemy opisy:
```{r}
wyniki <- ifelse(wektor4 > 0, "Dodatnia", "Ujemna")
```
...
Definiujemy wektor:
```{r}
wektor6 <- c(1, 2, 3, 4, 5, 6)
```

```{r}
indeksy_parzyste <- which(wektor6%%2==0)
```
...
```{r}
if (x%%3==0) {
  wynik <- "x jest podzielne przez 3"
} else if (x%%3!=0) {
  wynik <- "x jest niepodzielne przez 3"
} 
```
```{r}
print(wynik)
